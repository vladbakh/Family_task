...
const INITIAL_DATA = {
    familyName: "Семья Бахаудиновых",
    users: [
        {id: 1, name: "Папа", role: "adult", points: 150, completedTasks: 5},
        {id: 2, name: "Мама", role: "adult", points: 180, completedTasks: 6},
        {id: 3, name: "Ариша", role: "teen", points: 90, completedTasks: 8},
        {id: 4, name: "Алеша", role: "child", points: 60, completedTasks: 6}
    ],
    tasks: [
...
function loadData() {
    ...
    // Если данных нет или ошибка, возвращаем начальные данные
...
function drawTasksForAll() {
    ...
    if (results.length > 0) {
        ...
        showNotification("Нет доступных задач для распределения", "warning");
    }
...
function updateUserTasks() {
    ...
    // Добавляем обработчики событий для динамически созданных кнопок
    setTimeout(() => {
        ...
        taskElement.innerHTML = `
            <div>
                ...
                <div class="task-details">
                    ...
                    ${assignedToUser ? ` • Назначена: ${assignedToUser.name}` : ''}
                </div>
            </div>
            <div class="list-item-actions">
...
function spinWheel() {
    ...
    setTimeout(() => {
        ...
        if (task) {
            ...
            showNotification("Нет доступных задач для выбранного пользователя", "warning");
        }
